             ...
             [:form.col.s12
                         [:div.row
                             ...
                             [:div.input-field.col.s12
                                 ...
                                 [:label {:for "icon_text"} "Üzenet"]]


                             [:button.btn.right.transparent.white-text {:type "button" :on-click #(check-email @email-atom)} "Üzenet küldése ->"]]]])))


(defn contact []
    (let []
        (reagent/create-class {
           ;:component-did-mount #(.tablebutton js/window js/document "script")
                               :reagent-render
                               (fn []
                                  [:div.row
